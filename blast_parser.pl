...
# Altered version :
# 
# Julie Cremaschi, Samuel Mondy (June 6, 2018) 
#
# This software is altered and does not give the original output.
# Altered parts are marked by a [!].
#

#
# Original version :
#
# Version 1.1.6 (November 19, 2015)
#
# Copyright (c) 2011-2015 Kirill Kryukov
#
...
#[!]
print "qname\tqlen\tsname\tslen\tframe\tlength\tident\tpositives\tgaps\tbits\texpect\tqstart\tqend\tsstart\tsend\n";
...
while (<>)
{
    ...
    if (/^\sIdentities/)
    {
        ...
        if (/^\sIdentities\s+=\s+(\d+)\/(\d+)\s+\(\d+\%\)/) { ($ident,$length) = ($1,$2); }
        if (/\s+Positives\s+=\s+(\d+)\/\d+\s+\(\d+\%\)/) { ($positives) = ($1); }
        if (/Gaps\s+=\s+(\d+)\//) { $gaps = $1; } else { $gaps = 0; }
        if ($length < 0) { print STDERR "Error: unexpected BLAST ' Identities = ' line format: $_\n"; }
        next;
    }
...
sub flush_hit
{
    ...
    #[!]
    #print "          $frame  $length($ident,$positives,$gaps) - $bits($expect)  $qstart..$qend - $sstart..$send\n";
    print "$qname\t$qlen\t$sname\t$slen\t$frame\t$length\t$ident\t$positives\t$gaps\t$bits\t$expect\t$qstart\t$qend\t$sstart\t$send\n";
    $hchanged = 0;
    $frame = '';
